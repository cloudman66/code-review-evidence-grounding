...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--dev-aux-cache", required=True)
    parser.add_argument("--test-aux-cache", required=True)
    parser.add_argument(
        "--policies",
...
def build_policy(name: str):
    ...
    if name == "gt40_or_quoted_no_or_mention":
        return lambda sample: len(sample["contexts"]) > 40 or not bool(extract_quoted_spans(sample["comment"])) or "@" in sample["comment"]
...
def run_split(
    *,
    ...
    for row in prediction_rows:
        ...
        aux_group = aux_group_map[sample_id]
        base_ranked = rank_contexts(base_group["context_ids"], base_group["scores"])
        aux_ranked = rank_contexts(aux_group["context_ids"], aux_group["scores"])
...
def build_report(results: list[dict], best: dict) -> str:
    lines = ["# Selective Top-K Hybrid Search", "", "## Best", ""]
...
def main() -> None:
    ...
    dev_aux_group_map = cache_to_group_map(Path(args.dev_aux_cache))
    test_aux_group_map = cache_to_group_map(Path(args.test_aux_cache))
    ...
    for policy_name in [item.strip() for item in args.policies.split(",") if item.strip()]:
        dev_metrics, dev_predictions, applied_dev = run_split(
            ...
            aux_group_map=dev_aux_group_map,
        ...
        test_metrics, test_predictions, applied_test = run_split(
            ...
            aux_group_map=test_aux_group_map,
            policy_name=policy_name,
        )
        row = {
            "policy": policy_name,
